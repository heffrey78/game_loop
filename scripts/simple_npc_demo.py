...
def demonstrate_archetype_system():
    ...
    for archetype, data in archetypes.items():
        ...
        print(
            f"  Best Locations: {[loc for loc, aff in data['location_affinities'].items() if aff > 0.7]}"
        )
...
def demonstrate_npc_generation():
    ...
    knowledge = MockNPCKnowledge(
        ...
        world_knowledge={
            "economy": "trade is flourishing",
            "politics": "stable region",
        },
        local_knowledge={
            "village": "knows all families",
            "events": "market day schedule",
        },
    )
...
def demonstrate_embedding_search():
    ...
    npcs = [
        {
            "name": "Elara Goldweaver",
            "archetype": "merchant",
            "traits": "persuasive, business-minded",
        },
        {"name": "Old Henrik", "archetype": "hermit", "traits": "wise, reclusive"},
        {
            "name": "Captain Marcus",
            "archetype": "guard",
            "traits": "dutiful, protective",
        },
    ]
    ...
    for search in searches:
        ...
        if "trader" in search or "commerce" in search:
            print("     → Best Match: Elara Goldweaver (merchant)")
        elif "wise" in search or "knowledge" in search:
            print("     → Best Match: Old Henrik (hermit)")
        elif "protective" in search or "order" in search:
            print("     → Best Match: Captain Marcus (guard)")
