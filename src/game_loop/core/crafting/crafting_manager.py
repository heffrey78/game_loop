...
class CraftingManager:
        ...
        try:
            ...
            for component, quantity in recipe.required_components.items():
                ...
                if not source_inventory:
                    return False, {"error": f"No source specified for component {component}"}
                ...
                if self.inventory:
                    ...
                    if not success:
                        ...
                        return False, {"error": f"Failed to reserve {component}: {result.get('error')}"}
            ...
            station_bonuses = await self._calculate_station_bonuses(recipe, available_stations)
    ...
    async def process_crafting_step(
        ...
        try:
            ...
            if step_successful:
                ...
                session["progress"] = max(0.0, session["progress"] - step_progress * 0.5)
            ...
            if step_successful:
                ...
                result["failure_reason"] = await self._determine_failure_reason(session, step_data)
    ...
    async def complete_crafting_session(self, session_id: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            final_success_probability = await self.calculate_crafting_success_probability(
                recipe.recipe_id, 
    ...
    async def cancel_crafting_session(
        ...
        try:
            ...
            progress = session["progress"]
            actual_recovery = recovery_percentage * (1.0 - progress * 0.5)  # Less recovery if more progress
                ...
                for pattern in patterns:
                    if await self._matches_discovery_pattern(components, pattern):
                        ...
                        if discovered_recipe:
                            # Add to registry
                            self._recipe_registry[discovered_recipe.recipe_id] = discovered_recipe
                            
                            logger.info(f"Discovered new recipe: {discovered_recipe.name}")
    ...
    async def validate_crafting_requirements(
        ...
        recipe_id: str, 
        crafter_id: str,
        available_components: Dict[str, int]
    ) -> Tuple[bool, List[str]]:
        """
        ...
        try:
            ...
            for component, required_quantity in recipe.required_components.items():
                ...
                if available_quantity < required_quantity:
                    errors.append(f"Need {required_quantity} {component}, have {available_quantity}")
            ...
            if recipe.crafting_stations:
                station_available = any(
                    station in available_stations for station in recipe.crafting_stations
                )
                if not station_available:
                    errors.append(f"Requires one of: {', '.join(recipe.crafting_stations)}")
            ...
            if current_energy < recipe.energy_cost:
                errors.append(f"Insufficient energy: need {recipe.energy_cost}, have {current_energy}")
    ...
    async def calculate_crafting_success_probability(
        ...
        try:
            ...
            if component_quality:
                ...
                quality_modifier = (avg_quality - 0.5) * 0.2  # Quality around 0.5 is neutral
    ...
    async def get_available_recipes(
        ...
        try:
            ...
            craftable_recipes.sort(key=lambda r: (r.complexity.value, -r.base_success_chance))
        ...
        try:
            ...
            for i, comp1 in enumerate(components):
                for j, comp2 in enumerate(components[i + 1:], i + 1):
                    compatibility = await self._calculate_component_compatibility(comp1, comp2)
                    compatibility_matrix[f"{comp1}+{comp2}"] = compatibility
                    overall_compatibility += compatibility
                    total_pairs += 1
            
            avg_compatibility = overall_compatibility / total_pairs if total_pairs > 0 else 1.0
            ...
            return {
                ...
                "compatibility_rating": (
                    "excellent" if avg_compatibility > 0.8 else
                    "good" if avg_compatibility > 0.6 else
                    "fair" if avg_compatibility > 0.4 else
                    "poor"
                ),
            }
    ...
    async def _rollback_component_reservations(self, reserved_components: Dict[str, Dict[str, Any]]) -> None:
    ...
    async def _create_crafting_products(self, session: Dict[str, Any]) -> Dict[str, int]:
    ...
    async def _create_crafting_byproducts(self, session: Dict[str, Any]) -> Dict[str, int]:
        """Create byproducts from crafting."""
        recipe = session["recipe"]
        return recipe.byproducts

    async def _award_crafting_experience(self, session: Dict[str, Any]) -> Dict[str, float]:
        """Award skill experience for successful crafting."""
        recipe = session["recipe"]
        return recipe.skill_experience

    async def _generate_failure_salvage(self, session: Dict[str, Any]) -> Optional[Dict[str, int]]:
    ...
    async def _record_crafting_attempt(
        ...
        if len(self._crafting_history[crafter_id]) > max_history:
            self._crafting_history[crafter_id] = self._crafting_history[crafter_id][-max_history:]
    ...
    async def _record_crafting_cancellation(
        ...
        await self._record_crafting_attempt(session, {
            "success": False,
            "cancelled": True,
    ...
    async def _generate_recipe_from_pattern(
        ...
        return CraftingRecipe(
            ...
            required_components={comp: 1 for comp in components},
            optional_components={},
