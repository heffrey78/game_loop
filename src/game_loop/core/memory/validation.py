...
def validate_probability(value: Any, field_name: str, allow_none: bool = False) -> float:
...
def validate_relationship_impact(value: Any, field_name: str = "relationship_impact") -> float:
...
def validate_emotional_memory_type(value: Any, field_name: str = "emotional_type") -> EmotionalMemoryType:
...
def validate_protection_level(value: Any, field_name: str = "protection_level") -> MemoryProtectionLevel:
...
def validate_positive_number(value: Any, field_name: str, allow_zero: bool = True) -> Union[int, float]:
...
def validate_string_content(value: Any, field_name: str, 
...
def validate_keywords_list(value: Any, field_name: str = "keywords", 
                         max_keywords: int = 50, max_keyword_length: int = 100) -> List[str]:
    ...
    for i, keyword in enumerate(value):
        try:
            ...
            validated_keyword = sanitize_user_content(validated_keyword, max_keyword_length)
...
def validate_personality_traits(traits: Any, field_name: str = "traits") -> Dict[str, float]:
    ...
    for trait_name, trait_value in traits.items():
        ...
        try:
            trait_value = validate_probability(trait_value, f"{field_name}.{trait_name}")
...
def validate_mood_accessibility(accessibility: Any, field_name: str = "mood_accessibility") -> Dict[MoodState, float]:
    ...
    for mood_key, access_value in accessibility.items():
        try:
            ...
            access_value = validate_probability(access_value, f"{field_name}.{mood_state.value}")
...
class InputValidator:
    ...
    def validate_mood_update_input(self, **kwargs) -> Dict[str, Any]:
        ...
        if 'intensity' in kwargs:
            validated['intensity'] = validate_intensity(kwargs['intensity'], 'intensity')
    ...
    def handle_validation_error(self, error: Exception, context: str = "") -> ValidationError:
