...
class MemoryPerformanceValidator:
    ...
    def __init__(self, config: MemoryAlgorithmConfig | None = None):
            ...
            self.test_results.append(
                PerformanceTestResult(
                    ...
                    error_message=(
                        None
                        if passed
                        else f"Average time {avg_time_ms:.3f}ms exceeds {target_ms}ms target"
                    ),
                    additional_metrics={
            ...
            self.test_results.append(
                PerformanceTestResult(
                    ...
                    error_message=(
                        None
                        if passed
                        else "Concurrent processing failed performance or accuracy requirements"
                    ),
                    additional_metrics={
                        ...
                        "avg_time_per_op_ms": round(
                            total_time_ms / concurrent_count, 3
                        ),
                    },
                )
            )
    ...
    async def _test_emotional_weighting_accuracy(self) -> None:
        ...
        try:
            ...
            for i, test_case in enumerate(test_cases):
                result = await service.analyze_memory_emotional_weight(
                    test_case["content"]
                )
                ...
                if min_expected <= actual_weight <= max_expected:
                    ...
                    print(
                        f"    ❌ {test_case['description']}: expected {min_expected}-{max_expected}, got {actual_weight:.3f}"
                    )
            ...
            self.test_results.append(
                PerformanceTestResult(
                    ...
                    error_message=(
                        None
                        if passed
                        else f"Emotional weighting accuracy {accuracy:.1%} below {target_accuracy:.1%} target"
                    ),
                    additional_metrics={
    ...
    async def _test_clustering_accuracy(self) -> None:
        ...
        try:
            ...
            clustering_result = await service.cluster_npc_memories(
                npc_id=uuid.uuid4(), memory_data=test_memories, target_clusters=3
            )
            ...
            target_score = (
                0.25  # Adjusted clustering quality threshold based on test data
            )
            ...
            self.test_results.append(
                PerformanceTestResult(
                    ...
                    error_message=(
                        None
                        if passed
                        else f"Clustering quality {silhouette_score:.3f} below {target_score:.3f} target"
                    ),
                    additional_metrics={
    ...
    async def _test_personality_modifiers(self) -> None:
        ...
        try:
            ...
            personality_distinct = (
                detail_oriented > forgetful
                and emotional > forgetful
                and abs(detail_oriented - forgetful) > 0.1
            )
            ...
            self.test_results.append(
                PerformanceTestResult(
                    ...
                    error_message=(
                        None
                        if passed
                        else "Personality modifiers not producing distinct behaviors"
                    ),
                    additional_metrics={
                        "personality_results": results,
                        "confidence_spread": round(
                            max(results.values()) - min(results.values()), 3
                        ),
                    },
                )
            )
    ...
    async def _test_batch_processing_performance(self) -> None:
        ...
        try:
            ...
            # Process batch
            start_time = time.perf_counter()
            result = await service.process_memory_batch(
                memory_batch, include_clustering=False
            )
            ...
            passed = (
                processing_time_ms <= target_time_ms
                and result.memories_processed == batch_size
            )
            ...
            self.test_results.append(
                PerformanceTestResult(
                    ...
                    error_message=(
                        None
                        if passed
                        else f"Batch processing time {processing_time_ms:.0f}ms exceeds {target_time_ms}ms target"
                    ),
                    additional_metrics={
                        "memories_processed": result.memories_processed,
                        "avg_time_per_memory_ms": round(
                            processing_time_ms / batch_size, 3
                        ),
    ...
    def _create_test_memory_dataset(self) -> list[dict[str, Any]]:
        ...
        for group_idx, (group, expected_emotion) in enumerate(
            zip(all_groups, expected_emotions, strict=False)
        ):
                ...
                memories.append(
                    {
                        ...
                        "emotional_weight": expected_emotion
        ...
        for expected_group, expected_members in expected_groups.items():
            ...
            for member in expected_members:
                if member in cluster_assignments:
                    ...
                    coherence = len(
                        [m for m in expected_members if m in same_cluster_members]
        ...
        performance_summary = {
            ...
            "test_categories": {
                ...
                "functionality": [
                    ...
                    if "Effectiveness" in r.test_name or "Handling" in r.test_name
                ],
            },
            "critical_failures": [
                r
                for r in self.test_results
                if not r.passed and "Performance" in r.test_name
            ],
        }
    ...
    def print_validation_report(self, report: ValidationReport) -> None:
        ...
        print(
            f"   Timestamp: {report.test_timestamp.strftime('%Y-%m-%d %H:%M:%S UTC')}"
        )
        print(
            f"   Duration: {report.performance_summary['validation_duration_seconds']:.1f}s"
        )
        print(
        ...
        for result in report.test_results:
            ...
            print(
                f"      Target: {result.target_metric} | Actual: {result.actual_metric}"
            )
            ...
            if result.additional_metrics:
                metrics_str = ", ".join(
                    [f"{k}: {v}" for k, v in result.additional_metrics.items()]
                )
        ...
        if report.overall_success:
            ...
            print(
                f"   Failed tests: {[r.test_name for r in report.test_results if not r.passed]}"
            )
