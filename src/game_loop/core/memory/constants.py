...
class EmotionalMemorySystemConfig:
    ...
    def get_threshold(
        self, category: str, threshold_name: str, default: float = 0.5
    ) -> float:
    ...
    def get_cache_config(self, cache_name: str) -> Dict[str, int]:
        ...
        return {
            "size": self.cache_sizes.get(
                cache_name, EmotionalThresholds.DEFAULT_CACHE_SIZE
            ),
            "ttl": self.cache_ttls.get(
                cache_name, EmotionalThresholds.DEFAULT_CACHE_TTL
            ),
        }
