...
class EmotionalWeightingAnalyzer:
    ...
    EMOTIONAL_KEYWORDS = {
        ...
        'joy': {
            'keywords': {'happy', 'joy', 'excited', 'delighted', 'thrilled', 'elated', 
                        'cheerful', 'pleased', 'glad', 'wonderful', 'amazing', 'fantastic'},
            'intensity': 0.8
        },
        'love': {
            'keywords': {'love', 'adore', 'cherish', 'treasure', 'devoted', 'caring', 
                        'affection', 'fondness', 'dear', 'precious', 'beloved'},
            'intensity': 0.9
        },
        'gratitude': {
        ...
        'anger': {
            'keywords': {'angry', 'furious', 'rage', 'mad', 'irritated', 'annoyed', 
                        'frustrated', 'outraged', 'livid', 'irate', 'hostile'},
            'intensity': 0.9
        },
        'fear': {
            'keywords': {'afraid', 'scared', 'terrified', 'frightened', 'anxious', 
                        'worried', 'nervous', 'panic', 'dread', 'horror', 'terror'},
            'intensity': 0.95  # Fear has highest retention for survival
        },
        'sadness': {
            'keywords': {'sad', 'depressed', 'grief', 'sorrow', 'misery', 'despair', 
                        'heartbroken', 'devastated', 'mourning', 'melancholy'},
            'intensity': 0.85
        },
        'betrayal': {
            'keywords': {'betrayed', 'deceived', 'lied', 'cheated', 'backstabbed', 
                        'disappointed', 'let down', 'trust', 'broken promise'},
            'intensity': 0.9
        },
        
        # Relationship emotions
        'trust': {
            'keywords': {'trust', 'reliable', 'dependable', 'honest', 'truthful', 
                        'loyal', 'faithful', 'integrity', 'confide'},
            'intensity': 0.75
        },
        'conflict': {
            'keywords': {'argue', 'fight', 'dispute', 'conflict', 'disagree', 
                        'quarrel', 'clash', 'confrontation'},
            'intensity': 0.8
        }
    }
    ...
    EMOTIONAL_AMPLIFIERS = {
        ...
        'utterly', 'deeply', 'intensely', 'overwhelmingly', 'profoundly'
    }
    
    # Relationship impact indicators
    RELATIONSHIP_INDICATORS = {
        'positive': {
            'keywords': {'helped', 'supported', 'saved', 'protected', 'assisted', 
                        'encouraged', 'comforted', 'befriended', 'trusted'},
            'impact': 0.8
        },
        'negative': {
            'keywords': {'hurt', 'betrayed', 'abandoned', 'ignored', 'rejected', 
                        'insulted', 'threatened', 'attacked', 'deceived'},
            'impact': -0.8
        },
        'significant': {
            'keywords': {'first time', 'never forget', 'always remember', 'changed everything',
                        'life changing', 'turning point', 'milestone', 'special moment'},
            'impact': 0.9
        }
    }
    ...
    def analyze_emotional_weight(
        ...
        self._performance_stats['avg_time_ms'] = (
            (self._performance_stats['avg_time_ms'] * (total_analyses - 1) + processing_time_ms)
            / total_analyses
        )
    ...
    def _perform_emotional_analysis(
        ...
        participant_info: Optional[Dict],
    ) -> EmotionalAnalysisResult:
        ...
        emotional_keywords, keyword_intensity = self._detect_emotional_keywords(cleaned_text)
    ...
    def _detect_emotional_keywords(self, text: str) -> Tuple[List[str], float]:
        ...
        normalized_intensity = min(total_intensity / 3.0, 1.0)  # Cap at reasonable level
    ...
    def _calculate_analysis_confidence(
        ...
        # Combined confidence
        confidence = (length_confidence + keyword_confidence + sentiment_confidence) / 3.0
        
        return max(0.1, min(1.0, confidence))  # Minimum 0.1 confidence
    
    def _create_cache_key(
        self, message_content: str, conversation_context: Optional[Dict]
    ) -> str:
    ...
    def get_performance_stats(self) -> Dict[str, float]:
        ...
        total_requests = self._performance_stats['analyses'] + self._performance_stats['cache_hits']
        cache_hit_rate = (
