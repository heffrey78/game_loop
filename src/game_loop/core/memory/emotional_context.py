...
from .constants import EmotionalThresholds, EmotionalTypeThresholds, MoodTransitionPatterns
from .emotional_analysis import EmotionalAnalysisResult, EmotionalWeightingAnalyzer
from .exceptions import (
    EmotionalAnalysisError, InvalidEmotionalDataError, handle_emotional_memory_error
)
from .validation import (
    validate_uuid, validate_probability, validate_string_content, 
    validate_mood_state, validate_emotional_memory_type, default_validator
)
...
class EmotionalMemoryContextEngine:
    ...
    def __init__(
        ...
        self.config = config
        self.emotional_analyzer = emotional_analyzer or EmotionalWeightingAnalyzer(config)
        ...
        self._max_cache_size = config.get_processing_limit('max_cache_size', EmotionalThresholds.DEFAULT_CACHE_SIZE) if hasattr(config, 'get_processing_limit') else EmotionalThresholds.DEFAULT_CACHE_SIZE
    ...
    async def analyze_emotional_significance(
        ...
        try:
            ...
            if not exchange or not exchange.exchange_id:
                raise InvalidEmotionalDataError("Exchange must have a valid exchange_id")
        ...
        try:
            ...
            significance = await self._analyze_comprehensive_significance(
                exchange, conversation_context, npc_personality, basic_analysis, additional_context
            )
            
            # Cache result with size management
            if len(self._significance_cache) >= self._max_cache_size:
                # Remove oldest entries (simple FIFO)
                oldest_keys = list(self._significance_cache.keys())[:int(self._max_cache_size * 0.1)]
    ...
    async def _classify_emotional_memory_type(
        ...
        trauma_keywords = {
            "trauma", "traumatic", "nightmare", "terrified", "horrified", 
            "devastated", "shattered", "broken", "destroyed", "ruined",
            "abused", "attacked", "betrayed", "abandoned", "rejected"
        }
        if any(keyword in text for keyword in trauma_keywords) and emotional_weight > EmotionalThresholds.TRAUMA_WEIGHT_THRESHOLD:
            return EmotionalMemoryType.TRAUMATIC
        
        # Check for peak positive experiences
        peak_positive_keywords = {
            "best day", "happiest", "amazing", "incredible", "unforgettable",
            "perfect", "wonderful", "magical", "dream come true", "blessed"
        }
        if any(keyword in text for keyword in peak_positive_keywords) and emotional_weight > EmotionalThresholds.PEAK_EXPERIENCE_THRESHOLD:
            return EmotionalMemoryType.PEAK_POSITIVE
        
        # Check for core attachment/relationship moments
        attachment_keywords = {
            "love", "loved", "care", "caring", "devoted", "cherish",
            "family", "parent", "child", "partner", "best friend"
        }
        if any(keyword in text for keyword in attachment_keywords) and relationship_impact > EmotionalThresholds.HIGH_RELATIONSHIP_IMPACT:
            return EmotionalMemoryType.CORE_ATTACHMENT
        
        # Check for formative experiences
        formative_keywords = {
            "first time", "learned", "realized", "understood", "changed me",
            "grew up", "matured", "became", "discovered", "awakened"
        }
        if any(keyword in text for keyword in formative_keywords) and emotional_weight > EmotionalThresholds.FORMATIVE_WEIGHT_THRESHOLD:
            return EmotionalMemoryType.FORMATIVE
        
        # Check for significant loss
        loss_keywords = {
            "died", "death", "lost", "goodbye", "farewell", "ended",
            "grief", "mourning", "miss", "gone", "never again"
        }
        if any(keyword in text for keyword in loss_keywords) and emotional_weight > EmotionalThresholds.FORMATIVE_WEIGHT_THRESHOLD:
            return EmotionalMemoryType.SIGNIFICANT_LOSS
        
        # Check for breakthrough moments
        breakthrough_keywords = {
            "achieved", "accomplished", "succeeded", "breakthrough", "victory",
            "proud", "overcome", "conquered", "mastered", "breakthrough"
        }
        if any(keyword in text for keyword in breakthrough_keywords) and emotional_weight > EmotionalThresholds.FORMATIVE_WEIGHT_THRESHOLD:
            return EmotionalMemoryType.BREAKTHROUGH
        
        # Check for conflict
        conflict_keywords = {
            "argue", "argued", "fight", "fought", "angry", "mad",
            "disagreed", "conflict", "confrontation", "quarrel"
        }
        if any(keyword in text for keyword in conflict_keywords) and emotional_weight > EmotionalThresholds.MODERATE_SIGNIFICANCE:
            return EmotionalMemoryType.CONFLICT
        
        # Check for trust events
        trust_keywords = {
            "trust", "trusted", "betrayed", "loyal", "faithful", 
            "honest", "lied", "deceived", "reliable", "depend"
        }
        if any(keyword in text for keyword in trust_keywords) and relationship_impact > EmotionalThresholds.FORMATIVE_WEIGHT_THRESHOLD:
            return EmotionalMemoryType.TRUST_EVENT
        
        # Default categorization based on emotional weight
        if basic_analysis.sentiment_score > EmotionalThresholds.LOW_SIGNIFICANCE and emotional_weight > (EmotionalThresholds.MODERATE_SIGNIFICANCE - 0.1):
            return EmotionalMemoryType.EVERYDAY_POSITIVE
        elif basic_analysis.sentiment_score < -EmotionalThresholds.LOW_SIGNIFICANCE and emotional_weight > (EmotionalThresholds.MODERATE_SIGNIFICANCE - 0.1):
            return EmotionalMemoryType.ROUTINE_NEGATIVE
        else:
            return EmotionalMemoryType.EVERYDAY_POSITIVE  # Default to positive for neutral
    ...
    def _calculate_personal_relevance(
        ...
        relevance = base_relevance * (EmotionalThresholds.MODERATE_SIGNIFICANCE + emotional_sensitivity)
    ...
    async def _assess_formative_influence(
        ...
        relationship_amplifier = 1.0 + float(conversation_context.relationship_level) * 0.5
        ...
        experience_modifier = 1.0 + (1.0 - float(conversation_context.relationship_level)) * 0.2
        
        formative_influence = base_influence * personal_relevance * relationship_amplifier * experience_modifier
    ...
    def _determine_protection_level(
        ...
        base_protection = type_protection.get(emotional_type, MemoryProtectionLevel.PUBLIC)
        
        # Upgrade protection based on formative influence
        if formative_influence > EmotionalThresholds.HIGH_SIGNIFICANCE_THRESHOLD and base_protection != MemoryProtectionLevel.TRAUMATIC:
    ...
    def _calculate_mood_accessibility(
        ...
        base_accessibility = EmotionalThresholds.MODERATE_SIGNIFICANCE  # Neutral baseline
        ...
        for mood in accessibility:
            ...
            accessibility[mood] = base_accessibility + mood_effect * (1.0 + emotional_sensitivity)
    ...
    def _calculate_triggering_potential(
        ...
        relationship_amplifier = 1.0 + float(conversation_context.relationship_level) * 0.3
    ...
    def _identify_contributing_factors(
        ...
        if basic_analysis.emotional_weight > EmotionalThresholds.TRAUMA_WEIGHT_THRESHOLD:
        ...
        if abs(basic_analysis.relationship_impact) > EmotionalThresholds.FORMATIVE_WEIGHT_THRESHOLD:
            factors.append("significant_relationship_impact")
        
        if basic_analysis.emotional_keywords:
            factors.append(f"emotional_keywords: {', '.join(basic_analysis.emotional_keywords[:3])}")
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        total_requests = self._performance_stats["significance_analyses"] + self._performance_stats["cache_hits"]
        cache_hit_rate = (
        ...
        return {
            "total_significance_analyses": self._performance_stats["significance_analyses"],
