...
class MemoryConfidenceCalculator:
    ...
    def calculate_confidence(
        ...
        if memory_age_days < 0:
            raise ValueError(f"memory_age_days cannot be negative, got {memory_age_days}")
        if not (0.0 <= emotional_weight <= 1.0):
            raise ValueError(f"emotional_weight must be 0.0-1.0, got {emotional_weight}")
            
        # Use cached calculation if available
        cache_key = self._create_cache_key(
            base_confidence, memory_age_days, emotional_weight,
            access_count, personality_decay_modifier, personality_emotional_sensitivity
        )
        ...
        effective_decay_rate = (
            self.config.base_decay_rate * personality_decay_modifier
        )
        
        if emotional_amplifier > 1.0:
            # Emotional memories decay slower
            age_decay = math.exp(-effective_decay_rate * memory_age_days / emotional_amplifier)
        else:
        ...
        if processing_time_ms > self.config.max_processing_time_ms:
            print(f"Warning: Confidence calculation took {processing_time_ms:.2f}ms "
    ...
    def _calculate_emotional_amplifier(
        ...
        amplifier = 1.0 + (adjusted_weight * (self.config.emotional_multiplier_max - 1.0))
    ...
    def _create_cache_key(
        self, base_confidence: float, memory_age_days: float,
        emotional_weight: float, access_count: int,
        personality_decay_modifier: float, personality_emotional_sensitivity: float
    ) -> str:
    ...
    def _get_cached_confidence(self, cache_key: str) -> Optional[float]:
    ...
    def batch_calculate_confidence(
        ...
        if avg_time_per_calc > self.config.max_processing_time_ms:
            print(f"Warning: Batch confidence calculation averaged {avg_time_per_calc:.2f}ms "
