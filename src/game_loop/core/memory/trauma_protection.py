...
from .emotional_context import EmotionalMemoryType, EmotionalSignificance, MoodState, MemoryProtectionLevel
from .emotional_preservation import EmotionalMemoryRecord, EmotionalRetrievalQuery
from .exceptions import (
    TraumaProtectionError, InvalidEmotionalDataError, SecurityError,
...
class TraumaMemoryProfile:
    ...
    trigger_sensitivity: float = EmotionalThresholds.MODERATE_SIGNIFICANCE       # 0.0-1.0 sensitivity to triggers
    
    # Protection mechanisms active
    active_protections: Set[ProtectionMechanism] = field(default_factory=set)
    protection_effectiveness: Dict[ProtectionMechanism, float] = field(default_factory=dict)
    
    # Therapeutic context
    therapeutic_progress: float = 0.0      # 0.0-1.0 progress in processing
    preferred_approach: Optional[TherapeuticApproach] = None
    safe_memory_anchors: List[str] = field(default_factory=list)  # Safe memories for grounding
    
    # Thresholds and limits
    daily_trauma_exposure_limit: int = EmotionalThresholds.MAX_DAILY_TRAUMA_EXPOSURE   # Max trauma memories per day
...
class TraumaAccessRequest:
    ...
    urgency_level: float = EmotionalThresholds.MODERATE_SIGNIFICANCE          # 0.0-1.0 urgency of access
...
class TraumaProtectionEngine:
    ...
    async def evaluate_trauma_access_request(
        ...
        try:
            ...
            validate_positive_number(access_request.time_availability, "time_availability", allow_zero=True)
            
            # Validate trauma memory IDs
            for i, memory_id in enumerate(access_request.trauma_memory_ids):
                validate_string_content(memory_id, f"trauma_memory_ids[{i}]", min_length=1)
        ...
        try:
            ...
            decision = await self._make_access_decision(
                trauma_profile, access_request, risk_assessment, therapeutic_value, personality
            )
            ...
            logger.debug(f"Trauma access decision for NPC {npc_id}: {decision.access_granted}")
            return decision
    ...
    async def build_trauma_memory_profile(
        ...
        trauma_memories = [
            m.exchange_id for m in memories
        ...
        if trauma_memories:
            most_recent_trauma = max(
                m.preserved_at for m in memories
                if m.exchange_id in trauma_memories
            )
        ...
        safe_anchors = [
            m.exchange_id for m in memories
            if (m.emotional_significance.emotional_type in [
        ...
        profile = TraumaMemoryProfile(
            ...
            trigger_sensitivity=self._calculate_trigger_sensitivity(personality, trauma_severity),
            safe_memory_anchors=safe_anchors,
            preferred_approach=self._determine_preferred_therapeutic_approach(personality),
        )
        
        # Set up initial protection mechanisms
        profile.active_protections = self._initialize_protection_mechanisms(profile, personality)
        
        # Calculate protection effectiveness
        for protection in profile.active_protections:
            effectiveness = self._calculate_protection_effectiveness(protection, personality, trauma_severity)
            profile.protection_effectiveness[protection] = effectiveness
        
        # Store profile
        self._trauma_profiles[npc_id_str] = profile
        
        logger.debug(f"Built trauma profile for NPC {npc_id}: {len(trauma_memories)} trauma memories")
        return profile
    ...
    async def provide_therapeutic_memory_access(
        ...
        if not trauma_profile.trauma_memories:
            return {
                ...
                "recommendations": ["Focus on building emotional resources and resilience"],
            }
        ...
        therapeutic_request = TraumaAccessRequest(
            ...
            trauma_memory_ids=trauma_profile.trauma_memories[:2],  # Start with limited exposure
    ...
    async def monitor_trauma_recovery_progress(
        ...
        regression_assessment = self._assess_regression_risk(trauma_profile, recovery_metrics)
    ...
    async def _assess_trauma_access_risk(
        ...
        if profile.current_daily_exposure >= profile.daily_trauma_exposure_limit:
            ...
            risk_factors["excessive_daily_exposure"] = profile.current_daily_exposure / profile.daily_trauma_exposure_limit * 0.3
        ...
        vulnerability_traits = [
            "trauma_sensitive", "anxious", "emotional_sensitivity"
        ]
        ...
        risk_factors["personality_vulnerability"] = min(1.0, personality_vulnerability / len(vulnerability_traits))
    ...
    async def _make_access_decision(
        ...
        if self.enable_strict_protection:
            ...
            if therapeutic_value > 0.6 and overall_risk < 0.5:
                ...
                denied_memories = []
                rationale = "Access granted: high therapeutic value with acceptable risk"
        ...
        if access_granted:
            ...
            if overall_risk > 0.4:
                decision.time_limit = min(0.5, request.time_availability)  # Max 30 minutes
    ...
    async def _apply_protection_mechanisms(
        ...
        overall_risk = sum(decision.risk_assessment.values()) / len(decision.risk_assessment)
        if overall_risk > 0.7:
    ...
    async def _update_trauma_profile_post_decision(
        ...
        if decision.access_granted:
            ...
            if request.therapeutic_intent:
                ...
                profile.therapeutic_progress = min(1.0, profile.therapeutic_progress + progress_increment)
    ...
    def _determine_primary_trauma_response(
        ...
        avoidance_score = sum(personality.get_trait_strength(trait) for trait in avoidant_traits)
        hypervigilance_score = sum(personality.get_trait_strength(trait) for trait in hypervigilant_traits)
        dissociation_score = sum(personality.get_trait_strength(trait) for trait in dissociative_traits)
    ...
    def _calculate_trigger_sensitivity(self, personality: NPCPersonality, trauma_severity: float) -> float:
        """Calculate trauma trigger sensitivity."""
        
        sensitivity_traits = ["emotional_sensitivity", "trauma_sensitive", "anxious"]
        sensitivity_score = sum(personality.get_trait_strength(trait) for trait in sensitivity_traits)
    ...
    def _determine_preferred_therapeutic_approach(
        ...
        analytical_score = sum(personality.get_trait_strength(trait) for trait in analytical_traits)
        experiential_score = sum(personality.get_trait_strength(trait) for trait in experiential_traits)
        narrative_score = sum(personality.get_trait_strength(trait) for trait in narrative_traits)
    ...
    def _calculate_protection_effectiveness(
        ...
        if protection in [ProtectionMechanism.EMERGENCY_CONTAINMENT, ProtectionMechanism.SAFETY_CHECKS]:
            effectiveness *= (0.7 + trauma_severity * 0.3)  # More effective for severe trauma
    ...
    async def _create_therapeutic_framework(
        ...
        framework = {
            "approach": profile.preferred_approach.value if profile.preferred_approach else "resource_building",
    ...
    async def _assess_trigger_response(
        ...
        personality_modifier = 1.0 - ((resilience_score + emotional_regulation) / 2 * 0.3)
        ...
        responses = {
            TraumaResponseType.AVOIDANCE: ["topic_avoidance", "conversation_withdrawal", "emotional_numbing"],
            TraumaResponseType.HYPERVIGILANCE: ["increased_alertness", "threat_scanning", "jumpiness"],
            TraumaResponseType.INTRUSION: ["unwanted_memories", "flashback_risk", "thought_intrusions"],
            TraumaResponseType.DISSOCIATION: ["emotional_detachment", "unreality_feelings", "memory_gaps"],
            TraumaResponseType.NUMBING: ["emotional_flatness", "reduced_engagement", "apathy"],
            TraumaResponseType.FLOODING: ["emotional_overwhelm", "panic_response", "crisis_risk"],
        }
    ...
    async def _assess_recovery_metrics(
        ...
        if profile.protection_effectiveness:
            avg_effectiveness = sum(profile.protection_effectiveness.values()) / len(profile.protection_effectiveness)
        ...
        if profile.therapeutic_progress > 0.5:
            ...
            metrics["setback_frequency"] = max(0.0, 0.3 - profile.therapeutic_progress * 0.2)
    ...
    def _assess_regression_risk(
        ...
        if profile.last_crisis_event and (time.time() - profile.last_crisis_event) < 86400:  # Last 24 hours
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_decision_time_ms": round(self._performance_stats["avg_decision_time_ms"], 2),
