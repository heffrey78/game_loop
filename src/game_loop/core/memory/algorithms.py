...
from .emotional_analysis import EmotionalWeightingAnalyzer, EmotionalAnalysisResult
from .clustering import MemoryClusteringEngine, MemoryClusterData, ClusterResult
...
class MemoryProcessingResult:
    ...
    clustering_result: Optional[ClusterResult]
...
class MemoryAlgorithmService:
    ...
    def __init__(
        self,
        config: Optional[MemoryAlgorithmConfig] = None,
    ...
    async def process_memory_batch(
        ...
        npc_personality_config: Optional[Dict[str, float]] = None,
        ...
        try:
            ...
            personality_modifiers = self._extract_personality_modifiers(npc_personality_config)
            ...
            emotional_analysis = await self._process_emotional_analysis_batch(memory_data)
    ...
    async def calculate_single_memory_confidence(
        ...
        return self.confidence_calculator.calculate_confidence(
            ...
            personality_emotional_sensitivity=personality_modifiers['emotional_sensitivity'],
        )
    ...
    async def cluster_npc_memories(
        ...
        for memory in memory_data:
            ...
            cluster_data.append(MemoryClusterData(
    ...
    def should_memory_express_uncertainty(
        self, confidence: float, npc_personality_config: Optional[Dict[str, float]] = None
    ) -> bool:
        """
    ...
    def get_memory_uncertainty_expression(
        self, confidence: float, npc_personality_type: Optional[str] = None
    ) -> str:
        """
        ...
        """
        base_expression = self.confidence_calculator.get_uncertainty_expression(confidence)
        ...
        if npc_personality_type == 'analytical':
            ...
            return base_expression.replace('That rings a bell', 'Something stirs in my memory')
    ...
    async def _process_confidence_batch(
        ...
        confidence_scores = self.confidence_calculator.batch_calculate_confidence(batch_data)
        
        # Create result mapping
        for memory_id, confidence in zip(memory_ids, confidence_scores):
    ...
    async def _perform_memory_clustering(
        ...
        for memory in memory_data:
            ...
            confidence = confidence_updates.get(memory_id, memory.get('confidence_score', 1.0))
    ...
    def _extract_personality_modifiers(
        self, npc_personality_config: Optional[Dict[str, float]]
    ) -> Dict[str, float]:
        ...
        return {
            'decay_modifier': (
                ...
                if npc_personality_config else 1.0
            ),
    ...
    def get_service_performance_stats(self) -> Dict[str, Any]:
        ...
        if self._service_stats['total_operations'] > 0:
            success_rate = (
                ...
                self._service_stats['total_operations'] * 100
            )
