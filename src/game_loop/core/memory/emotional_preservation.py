...
from .affective_weighting import AffectiveWeight, AffectiveWeightingStrategy, MoodBasedAccessibility
...
from .exceptions import (
    MemoryPreservationError, InvalidEmotionalDataError, PerformanceError,
...
class EmotionalMemoryRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "mood_accessibility": {mood.value: weight for mood, weight in self.mood_accessibility.items()},
...
class EmotionalRetrievalResult:
    ...
    def get_by_significance(self, min_significance: float = 0.5) -> List[EmotionalMemoryRecord]:
        """Get records above significance threshold."""
        return [
            record for record in self.emotional_records
            if record.emotional_significance.overall_significance >= min_significance
        ]
    
    def get_by_type(self, emotional_type: EmotionalMemoryType) -> List[EmotionalMemoryRecord]:
...
class EmotionalPreservationEngine:
    ...
    async def preserve_emotional_context(
        ...
        try:
            ...
            if emotional_significance.confidence_score < 0 or emotional_significance.confidence_score > 1:
                raise InvalidEmotionalDataError("Confidence score must be between 0 and 1")
        ...
        try:
            ...
            async with self.session_factory.get_session() as session:
                ...
                emotional_context_db = EmotionalContextModel(
                    ...
                    participant_emotions={
                        ...
                        "mood_accessibility": {
                            mood.value: weight for mood, weight in (mood_accessibility or {}).items()
                        },
                    },
                    emotional_intensity=emotional_significance.intensity_score,
                    relationship_impact_score=abs(emotional_significance.relationship_impact),
                )
                ...
                emotional_record.emotional_context_id = str(emotional_context_db.context_id)
            
            # Cache the record
            self._emotional_records_cache[emotional_record.exchange_id] = emotional_record
            
            # Manage cache size
            if len(self._emotional_records_cache) > self.cache_size:
                # Remove oldest entries
                oldest_keys = list(self._emotional_records_cache.keys())[:-self.cache_size//2]
            ...
            self._performance_stats["avg_preservation_time_ms"] = (
                self._performance_stats["avg_preservation_time_ms"] * (total_preservations - 1)
                + processing_time_ms
            ) / total_preservations
            
            logger.debug(f"Preserved emotional context for exchange {exchange.exchange_id}")
            return emotional_record
            
        except Exception as e:
            error = handle_emotional_memory_error(e, "Error preserving emotional context")
    ...
    async def retrieve_emotional_memories(
        ...
        try:
            ...
            # Validate limits
            if query.max_results <= 0 or query.max_results > EmotionalThresholds.MAX_MEMORY_CAPACITY:
                raise InvalidEmotionalDataError(f"max_results must be between 1 and {EmotionalThresholds.MAX_MEMORY_CAPACITY}")
            
            # Validate temporal constraints
            if query.max_age_hours is not None:
                validate_positive_number(query.max_age_hours, "max_age_hours", allow_zero=True)
            if query.min_age_hours is not None:
                validate_positive_number(query.min_age_hours, "min_age_hours", allow_zero=True)
        ...
        try:
            async with self.session_factory.get_session() as session:
                ...
                base_query = (
                    ...
                    .join(ConversationExchange, 
                          EmotionalContextModel.exchange_id == ConversationExchange.exchange_id)
                    .where(ConversationExchange.conversation_id.in_(
                        select(ConversationExchange.conversation_id)
                        .where(ConversationExchange.speaker_id == npc_id)
                    ))
                )
                ...
                if query.significance_threshold > 0:
                    filters.append(
                        EmotionalContextModel.emotional_intensity >= query.significance_threshold
                    )
                
                # Protection level (would need to be stored in participant_emotions)
                # For now, using relationship_impact_score as proxy
                protection_threshold = self._get_protection_threshold(query.protection_level_max)
                if protection_threshold > 0:
                    filters.append(
                        EmotionalContextModel.relationship_impact_score <= protection_threshold
                    )
                
                # Apply temporal constraints
                if query.max_age_hours:
                    cutoff_time = datetime.now(timezone.utc) - timedelta(hours=query.max_age_hours)
                    filters.append(EmotionalContextModel.created_at >= cutoff_time)
                
                if query.min_age_hours:
                    cutoff_time = datetime.now(timezone.utc) - timedelta(hours=query.min_age_hours)
                    filters.append(EmotionalContextModel.created_at <= cutoff_time)
                ...
                if query.emotional_types:
                    emotional_records = [
                        record for record in emotional_records
                        if record.emotional_significance.emotional_type in query.emotional_types
                    ]
                ...
                result_metadata = self._calculate_result_metadata(emotional_records, query)
                ...
                if len(self._retrieval_cache) > self.cache_size // 2:
                    oldest_keys = list(self._retrieval_cache.keys())[:-self.cache_size//4]
                    for key in oldest_keys:
                ...
                self._performance_stats["avg_retrieval_time_ms"] = (
                    self._performance_stats["avg_retrieval_time_ms"] * (total_retrievals - 1)
                    + processing_time_ms
                ) / total_retrievals
                
                logger.debug(f"Retrieved {len(emotional_records)} emotional memories for NPC {npc_id}")
                return retrieval_result
                
        except Exception as e:
            error = handle_emotional_memory_error(e, "Error retrieving emotional memories")
    ...
    async def _build_emotional_records_from_contexts(
        ...
        for context in emotional_contexts:
            ...
            emotional_type_str = participant_emotions.get("emotional_type", "everyday_positive")
            protection_level_str = participant_emotions.get("protection_level", "public")
            ...
            emotional_significance = EmotionalSignificance(
                ...
                formative_influence=float(context.emotional_intensity) * 0.8,  # Estimate
                protection_level=protection_level,
                mood_accessibility={},  # Would need to be reconstructed
                decay_resistance=float(context.emotional_intensity) * 0.7,  # Estimate
                triggering_potential=float(context.relationship_impact_score) * 0.6,  # Estimate
            ...
            affective_weight = AffectiveWeight(
                ...
                recency_boost=1.0,
                relationship_amplifier=1.0 + float(context.relationship_impact_score) * 0.3,
                formative_importance=float(context.emotional_intensity) * 0.8,
                access_threshold=self._get_access_threshold_for_protection(protection_level),
                trauma_sensitivity=0.8 if emotional_type == EmotionalMemoryType.TRAUMATIC else 0.0,
            ...
            record = EmotionalMemoryRecord(
                ...
                retrieval_contexts=[query.target_mood.value] if query.target_mood else [],
            )
    ...
    def _calculate_result_metadata(
        ...
        avg_confidence = sum(r.emotional_significance.confidence_score for r in records) / len(records)
        ...
        dominant_emotion = max(type_counts, key=type_counts.get).value if type_counts else None
        ...
        if query.target_mood:
            therapeutic_values = [
                self._calculate_therapeutic_value_for_mood(r.emotional_significance, query.target_mood)
                for r in records
            ]
    ...
    def _get_protection_threshold(self, protection_level: MemoryProtectionLevel) -> float:
    ...
    def _get_access_threshold_for_protection(self, protection_level: MemoryProtectionLevel) -> float:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_preservation_time_ms": round(self._performance_stats["avg_preservation_time_ms"], 2),
            "avg_retrieval_time_ms": round(self._performance_stats["avg_retrieval_time_ms"], 2),
