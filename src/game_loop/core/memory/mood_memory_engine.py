...
from .exceptions import (
    MoodEngineError, InvalidEmotionalDataError, PerformanceError,
    handle_emotional_memory_error
)
from .validation import (
    validate_uuid, validate_probability, validate_mood_state,
...
class MoodDependentMemoryEngine:
    ...
    async def update_npc_mood(
        ...
        try:
            ...
            if not isinstance(transition_type, MoodTransition):
                raise InvalidEmotionalDataError(f"Invalid transition_type: {transition_type}")
        ...
        try:
            ...
            max_history_size = EmotionalThresholds.MAX_MEMORY_CAPACITY // 100  # 100 entries max
            if len(self._mood_history[npc_id_str]) > max_history_size:
                self._mood_history[npc_id_str] = self._mood_history[npc_id_str][-(max_history_size//2):]
            ...
            logger.debug(f"Updated mood for NPC {npc_id}: {new_mood.value} (intensity: {intensity:.2f})")
            return mood_record
    ...
    async def get_mood_adjusted_memories(
        ...
        mood_adjusted_query = self._adjust_query_for_mood(retrieval_query, current_mood_record)
        ...
        if include_therapeutic and self._should_include_therapeutic_memories(current_mood_record):
    ...
    def get_mood_history(self, npc_id: uuid.UUID, hours_back: float = 24.0) -> List[MoodStateRecord]:
        ...
        return [
            mood for mood in self._mood_history[npc_id_str]
    ...
    def _initialize_mood_patterns(self) -> Dict[MoodState, MoodMemoryPattern]:
        ...
        patterns[MoodState.JOYFUL] = MoodMemoryPattern(
            ...
            enhanced_types={"peak_positive": 0.3, "breakthrough": 0.2, "everyday_positive": 0.15},
            suppressed_types={"traumatic": -0.5, "significant_loss": -0.3, "conflict": -0.2},
        ...
        patterns[MoodState.MELANCHOLY] = MoodMemoryPattern(
            ...
            enhanced_types={"significant_loss": 0.4, "nostalgic": 0.3, "core_attachment": 0.2},
    ...
    def _calculate_mood_stability(self, npc_id: str, new_mood: MoodState, intensity: float) -> float:
        ...
        recent_moods = [m for m in mood_history[-10:] if time.time() - m.timestamp < 3600]
    ...
    def _predict_mood_duration(
        ...
        if mood in [MoodState.ANGRY, MoodState.EXCITED, MoodState.FEARFUL]:
            intensity_modifier = 1.0 - (intensity * 0.5)  # High intensity burns out faster
        else:
            intensity_modifier = 1.0 - (intensity * 0.2)  # Moderate effect for other moods
        ...
        predicted_duration = base_duration * intensity_modifier * stability_modifier * continuity_modifier
        return max(0.5, min(24.0, predicted_duration))  # Clamp between 30 minutes and 24 hours
    ...
    async def _calculate_accessibility_changes(
        ...
        changes["intensity_filter_change"] = pattern.emotional_intensity_filter * intensity
    ...
    def _adjust_query_for_mood(
        ...
        adjusted_query = EmotionalRetrievalQuery(
            ...
            significance_threshold=max(query.significance_threshold, pattern.emotional_intensity_filter),
    ...
    def _should_include_therapeutic_memories(self, mood_record: MoodStateRecord) -> bool:
    ...
    def _deduplicate_and_sort_memories(
        ...
        def relevance_score(memory: EmotionalMemoryRecord) -> float:
            ...
            mood_accessibility = memory.mood_accessibility.get(mood_record.mood_state, 0.5)
    ...
    def _calculate_mood_impact_prediction(
        ...
        memory_emotion_type = memory_record.emotional_significance.emotional_type
        memory_intensity = memory_record.emotional_significance.intensity_score
        memory_relationship_impact = memory_record.emotional_significance.relationship_impact
        ...
        # Stability impact
        stability_change = -abs(intensity_impact) * 0.3  # Accessing memories reduces stability
        ...
        return {
            ...
            "confidence": min(0.9, memory_record.emotional_significance.confidence_score + 0.1),
    ...
    def _calculate_mood_regulation_score(
        ...
        current_congruence = abs(self._calculate_mood_memory_congruence(current_mood, memory_type))
        target_congruence = self._calculate_mood_memory_congruence(target_mood, memory_type)
        ...
        regulation_score = regulation_potential * memory_strength * (1.0 / mood_distance)
    ...
    def _clear_npc_caches(self, npc_id: str) -> None:
        ...
        keys_to_remove = [key for key in self._mood_prediction_cache if key.startswith(npc_id)]
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        cache_hit_rate = (
            (self._performance_stats["cache_hits"] / (total_operations + self._performance_stats["cache_hits"]) * 100)
            if total_operations > 0 else 0.0
        )
        
        return {
            "mood_updates": self._performance_stats["mood_updates"],
            "accessibility_calculations": self._performance_stats["accessibility_calculations"],
            ...
            "total_mood_history": sum(len(history) for history in self._mood_history.values()),
