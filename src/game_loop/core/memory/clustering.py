...
class MemoryClusteringEngine:
    ...
    def _filter_memories_for_clustering(
        ...
        for memory in memories:
            ...
            if (
                ...
                and memory.confidence_score < self.config.min_confidence_for_clustering
            ):
                continue

            # Skip emotionally neutral memories (unless they have high confidence)
            if (
                memory.emotional_weight < self.config.min_emotional_weight
                and memory.confidence_score < 0.7
            ):
                continue
    ...
    def _determine_optimal_clusters(self, memories: list[MemoryClusterData]) -> int:
        ...
        return max(
            2, optimal_clusters
        )  # Always at least 2 clusters for meaningful grouping
    ...
    def _prepare_feature_matrix(self, memories: list[MemoryClusterData]) -> np.ndarray:
        ...
        feature_matrix[:, embedding_dim:] = scaler.fit_transform(
            feature_matrix[:, embedding_dim:]
        )
    ...
    def _calculate_cluster_sizes(self, cluster_labels: np.ndarray) -> dict[int, int]:
        ...
        return dict(zip(unique_labels.astype(int), counts.astype(int), strict=False))
    ...
    def _create_fallback_cluster_result(
        ...
        cluster_confidence = {
            i: memory.confidence_score for i, memory in enumerate(memories)
        }
        cluster_sizes = dict.fromkeys(range(len(memories)), 1)
    ...
    def _update_clustering_stats(self, result: ClusterResult) -> None:
        ...
        new_avg = (
            (current_avg * (total - 1)) + result.total_processing_time_ms
        ) / total
        ...
        if result.silhouette_score is not None:
            ...
            if successful > 1:
                new_silhouette = (
                    (current_silhouette * (successful - 1)) + result.silhouette_score
                ) / successful
            else:
    ...
    def get_clustering_stats(self) -> dict[str, float]:
        ...
        if self._clustering_stats["total_clusterings"] > 0:
            success_rate = (
                ...
                / self._clustering_stats["total_clusterings"]
                * 100
            )
        ...
        return {
            ...
            "avg_processing_time_ms": round(
                self._clustering_stats["avg_processing_time_ms"], 2
            ),
            "avg_silhouette_score": round(
                self._clustering_stats["avg_silhouette_score"], 3
            ),
        }
