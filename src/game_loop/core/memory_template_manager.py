...
class MemoryTemplateManager(TemplateManager):
    ...
    def generate_memory_reference(
        ...
        try:
            ...
            if "personality_styles" in self._template_cache:
                ...
                result = template.module.generate_personality_memory_reference(**context)
    ...
    def integrate_memory_in_dialogue(
        ...
        transitions = {
            "natural": ["That reminds me,", "Speaking of that,", "Oh, that brings back"],
    ...
    def _generate_confidence_based_reference(
        self, memory_content: str, confidence: float, emotional_weight: float, trust_level: float
    ) -> str:
        """Fallback confidence-based memory reference generation."""
        if confidence >= 0.7:
            if emotional_weight > 0.7:
                patterns = ["I'll never forget", "I clearly remember", "That moment is burned into my memory"]
            else:
                patterns = ["I remember clearly", "I'm certain that", "I recall exactly"]
        elif confidence >= 0.5:
            patterns = ["I believe", "From what I recall", "I'm pretty sure"]
        elif confidence >= 0.3:
            patterns = ["I think", "That rings a bell", "I seem to recall"]
        else:
            patterns = ["I might be wrong, but", "I could be imagining this", "There's something familiar"]
    ...
    def _basic_trust_filter(
        ...
        sensitive_types = ["shameful_secrets", "intimate_relationships", "trauma", "family_secrets"]
    ...
    def _regenerate_with_different_pattern(self, original: str, avoid_phrase: str) -> str:
