...
class InventoryManager:
    ...
    def __init__(self, object_manager: Any = None, physics_engine: Any = None, search_service: Any = None):
        """
    ...
    async def create_inventory(
        ...
        custom_constraints: Optional[List[InventoryConstraint]] = None
    ) -> str:
        """
        ...
        try:
            ...
            template = self._inventory_templates.get(template_name, self._create_default_template())
            ...
            if custom_constraints:
                for constraint in custom_constraints:
                    inventory["constraints"][constraint.constraint_type.value] = constraint
            
            self._inventories[inventory_id] = inventory
            self._inventory_history[inventory_id] = []
            
            logger.info(f"Created inventory {inventory_id} for {owner_id} using template {template_name}")
            return inventory_id
    ...
    async def add_item(
        ...
        try:
            ...
            if not is_valid:
                return False, {"error": "Constraint violations", "details": constraint_errors}
            ...
            # Update constraints
            await self._update_constraint_values(inventory_id, total_weight, total_volume, quantity)
            
            # Update metadata
            inventory["metadata"]["total_items"] += quantity
            
            # Record in history
            await self._record_inventory_action(inventory_id, "add_item", {
                "item_id": item_id,
                "quantity": quantity,
                "slot": target_slot,
    ...
    async def remove_item(
        ...
        try:
            ...
            target_slots = []
            if from_slot:
                if from_slot in inventory["slots"] and inventory["slots"][from_slot].item_id == item_id:
                    target_slots = [from_slot]
            else:
                # Find all slots containing this item
                target_slots = [
                    slot_id for slot_id, slot in inventory["slots"].items()
            ...
            if available_quantity < quantity:
                return False, {"error": f"Only {available_quantity} available, cannot remove {quantity}"}
            ...
            for slot_id in target_slots:
                ...
                removed_from_slots.append({
                    "slot_id": slot_id,
                    "quantity": remove_from_this_slot
                })
            ...
            # Record in history
            await self._record_inventory_action(inventory_id, "remove_item", {
                "item_id": item_id,
                "quantity": quantity,
    ...
    async def move_item(
        ...
        try:
            # Remove from source
            remove_success, remove_result = await self.remove_item(from_inventory, item_id, quantity)
            if not remove_success:
                return False
            
            # Add to target
            add_success, add_result = await self.add_item(to_inventory, item_id, quantity, to_slot)
            if not add_success:
            ...
            await self._record_inventory_action(from_inventory, "transfer_out", {
                "item_id": item_id,
                "quantity": quantity,
            ...
            await self._record_inventory_action(to_inventory, "transfer_in", {
                "item_id": item_id,
                "quantity": quantity,
        ...
        try:
            ...
            for slot in inventory["slots"].values():
                if slot.item_id and slot.quantity > 0:
                    ...
                    item_properties = await self._get_item_properties(slot.item_id)
                    if item_properties:
                        total_weight += item_properties.get("weight", 0.0) * slot.quantity
                        total_volume += item_properties.get("volume", 0.0) * slot.quantity
                        
                        category = item_properties.get("category", "misc")
                        item_categories[category] = item_categories.get(category, 0) + slot.quantity
            
            # Constraint utilization
            constraints_status = {}
            for constraint_type, constraint in inventory["constraints"].items():
                utilization = constraint.current / constraint.limit if constraint.limit > 0 else 0.0
            ...
            return {
                ...
                "slots_used": sum(1 for slot in inventory["slots"].values() if slot.item_id),
                ...
                for slot in inventory["slots"].values():
                    if slot.item_id:
                        ...
                        if item_properties and item_properties.get("perishable", False):
                            ...
                            new_condition = max(0.0, current_condition - (decay_rate * time_elapsed))
        ...
        try:
            ...
            for slot_id, slot in inventory["slots"].items():
                if slot.item_id is None and not slot.locked:
                    ...
                    if not slot.constraints or self._check_slot_compatibility(slot, item_category):
                        return slot_id
    ...
    async def _record_inventory_action(
        ...
        try:
            ...
            if len(self._inventory_history[inventory_id]) > max_history:
                self._inventory_history[inventory_id] = self._inventory_history[inventory_id][-max_history:]

        except Exception as e:
            logger.error(f"Error recording inventory action: {e}")

    async def _get_item_properties(self, item_id: str) -> Optional[Dict[str, Any]]:
    ...
    def _check_slot_compatibility(self, slot: InventorySlot, item_category: str) -> bool:
    ...
    def _passes_filters(self, item_match: Dict[str, Any], filters: Optional[Dict[str, Any]]) -> bool:
    ...
    def _initialize_default_constraints(self) -> None:
        ...
        async def validate_weight_constraint(constraint: InventoryConstraint, changes: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
            ...
            if "add_item" in changes:
                ...
                if new_total > constraint.limit:
                    return False, f"Weight limit exceeded: {new_total:.1f}/{constraint.limit:.1f} {constraint.unit}"
            return True, None
        
        async def validate_volume_constraint(constraint: InventoryConstraint, changes: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
            ...
            if "add_item" in changes:
                ...
                if new_total > constraint.limit:
                    return False, f"Volume limit exceeded: {new_total:.1f}/{constraint.limit:.1f} {constraint.unit}"
            return True, None
        
        async def validate_count_constraint(constraint: InventoryConstraint, changes: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
            """Validate count constraint."""
            if "add_item" in changes:
                additional_count = 1  # Each distinct item counts as 1
                new_total = constraint.current + additional_count
                if new_total > constraint.limit:
                    return False, f"Item count limit exceeded: {new_total}/{constraint.limit} {constraint.unit}"
            return True, None
        
        self._constraint_validators[InventoryConstraintType.WEIGHT] = validate_weight_constraint
        self._constraint_validators[InventoryConstraintType.VOLUME] = validate_volume_constraint
        self._constraint_validators[InventoryConstraintType.COUNT] = validate_count_constraint