...
class GenerationTriggerManager:
    ...
    async def analyze_action_for_triggers(
        ...
        try:
            ...
            if action_result.location_change or self._is_exploration_action(action_result):
            ...
            for trigger in filtered_triggers:
                trigger.priority_score = await self.calculate_generation_priority(
                    trigger, {"player_state": player_state, "action_result": action_result}
                )
            ...
            logger.info(f"Generated {len(filtered_triggers)} triggers from player action")
    ...
    async def evaluate_world_gaps(self, current_location_id: UUID) -> list[ContentGap]:
        ...
        try:
            ...
            empty_location_gaps = await self._evaluate_empty_location_gaps(current_location)
            ...
            for gap in gaps:
                gap.player_impact = await self._calculate_gap_impact(gap, current_location_id)
            
            # Sort by impact and severity
            gaps.sort(key=lambda g: (g.player_impact, self._severity_weight(g.severity)), reverse=True)
            
            logger.info(f"Identified {len(gaps)} content gaps around location {current_location_id}")
            return gaps
        ...
        try:
            ...
            if player_state:
                ...
                location_familiarity = len([
                    loc_id for loc_id in player_state.visited_locations 
            ...
            if trigger.location_id:
                ...
                if location:
                    ...
                    content_density = len(location.npcs) + len(location.objects) + len(location.connections)
            ...
            recent_generation_count = len([
                ...
                if t.trigger_type == trigger.trigger_type and 
                t.triggered_at > datetime.now() - timedelta(hours=1)
            ])
    ...
    async def should_generate_location(self, context: LocationGenerationContext) -> bool:
        ...
        try:
            ...
            current_location = self.world_state.locations.get(context.source_location_id)
            if not current_location:
                return False
    ...
    async def get_trigger_history(
        ...
        try:
            ...
            relevant_triggers = [
                ...
                if trigger.location_id == location_id and 
                trigger.triggered_at > cutoff_time
            ]
    ...
    def _is_exploration_action(self, action_result: ActionResult) -> bool:
        ...
        return any(cmd in (action_result.command or "").lower() for cmd in exploration_commands)
    ...
    async def _check_boundary_triggers(
        ...
        current_location_id = action_result.new_location_id or player_state.current_location_id
        ...
        if len(current_location.connections) <= 1:
            trigger = GenerationTrigger(
                ...
                session_id=action_result.processed_input or player_state.player_id,  # Fallback
    ...
    async def _check_interaction_triggers(
        ...
        if not action_result.success and "not found" in action_result.feedback_message.lower():
    ...
    async def _filter_cooldown_triggers(
        ...
        for trigger in triggers:
            ...
            similar_recent = any(
                ...
                t.location_id == trigger.location_id and
                t.triggered_at > recent_cutoff
    ...
    async def _evaluate_empty_location_gaps(self, location: Location) -> list[ContentGap]:
    ...
    async def _calculate_gap_impact(self, gap: ContentGap, current_location_id: UUID) -> float:
