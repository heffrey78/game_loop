...
class DynamicWorldCoordinator:
    ...
    async def process_player_action(
        ...
        try:
            ...
            logger.info("Processing player action for dynamic world generation")
            ...
            relevant_opportunities = [
                ...
                if any(
                    self._opportunity_matches_trigger(opp, trigger)
            ...
            if not await self._should_generate_now(
                player_state, relevant_opportunities
            ):
            ...
            pipeline_result = await self.coordinate_generation_pipeline(
                relevant_opportunities
            )
            ...
            if pipeline_result.success:
                ...
                # Update statistics
                self.generation_statistics["total_generated"] += len(
                    pipeline_result.generated_content
                )
        ...
        try:
            ...
            for prediction in interest_predictions:
                if prediction.interest_score > 0.6:  # High interest threshold
                    opportunity = GenerationOpportunity(
                        ...
                        opportunity_score=prediction.interest_score
            ...
            if context.current_location:
                exploration_opportunities = (
                    await self._evaluate_exploration_opportunities(
        ...
        try:
            if not self.generation_pipeline:
                ...
                return GenerationPipelineResult(
                    success=False, error_messages=["Pipeline not initialized"]
                )
            ...
            # Execute the plan
            pipeline_result = await self.generation_pipeline.execute_generation_plan(
                generation_plan
            )

            # Monitor quality of generated content
            if pipeline_result.success and pipeline_result.generated_content:
                await self._monitor_generated_content_quality(
                    pipeline_result.generated_content
                )
    ...
    async def monitor_generation_quality(
        ...
        try:
            ...
            quality_issues = await self.quality_monitor.detect_quality_issues(
                generated_content
            )
            ...
            avg_quality = sum(
                a.overall_quality_score for a in quality_assessments
            ) / len(quality_assessments)
            self._update_quality_statistics(avg_quality, quality_issues)

            logger.info(
                f"Monitored quality for {len(generated_content)} items, avg quality: {avg_quality:.2f}"
            )
    ...
    async def update_generation_preferences(
        self, quality_feedback: QualityFeedback
    ) -> bool:
        """
        ...
        try:
            ...
            if (
                hasattr(quality_feedback, "rating")
                and quality_feedback.rating is not None
                and quality_feedback.rating < 3
            ):
    ...
    async def get_world_generation_status(self) -> WorldGenerationStatus:
        ...
        try:
            ...
            successful_gens = self.generation_statistics.get(
                "successful_generations", 0
            )
            if successful_gens > 0:
                avg_time = self.generation_statistics.get("avg_generation_time", 0.0)
                status.average_generation_time = (
                    avg_time if isinstance(avg_time, (int, float)) else 0.0
                )
    ...
    async def _should_generate_now(
        ...
        if (
            last_generation
            and isinstance(last_generation, datetime)
            and datetime.now() - last_generation < self.generation_cooldown
        ):
            return False
    ...
    async def _monitor_generated_content_quality(
    ...
    def _update_quality_statistics(
        self, avg_quality: float, quality_issues: list
    ) -> None:
    ...
    async def _adjust_quality_thresholds(
        self, quality_feedback: "QualityFeedback"
    ) -> None:
