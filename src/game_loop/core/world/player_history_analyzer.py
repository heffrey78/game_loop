...
class PlayerHistoryAnalyzer:
    ...
    async def get_exploration_patterns(self, player_id: UUID) -> ExplorationPatterns:
        ...
        try:
            ...
            if visit_data:
                ...
                patterns.backtracking_frequency = (
                    await self._calculate_backtracking_frequency(player_id)
                )

                # Assess discovery thoroughness
                patterns.discovery_thoroughness = (
                    await self._assess_discovery_thoroughness(player_id)
                )
        ...
        try:
            ...
            if session_data:
                ...
                if metrics.session_duration > 0:
                    metrics.actions_per_minute = action_count / (
                        metrics.session_duration / 60
                    )
                ...
                metrics.satisfaction_indicators = (
                    await self._gather_satisfaction_indicators(player_id, session_id)
                )
    ...
    async def predict_player_interests(
        ...
        try:
            ...
            for content_type in content_types:
                ...
                base_interest = preferences.content_type_preferences.get(
                    content_type, 0.5
                )
            ...
            logger.info(
                f"Generated {len(predictions)} interest predictions for player {player_state.player_id}"
            )
            return predictions
    ...
    async def get_content_interaction_history(
        ...
        try:
            ...
            logger.info(
                f"Retrieved interaction history for player {player_id}, content type {content_type}"
            )
            return interactions
    ...
    async def update_preference_model(
        ...
        try:
            ...
            cache_keys_to_remove = [
                key
                for key in self.preference_cache.keys()
    ...
    async def _analyze_content_type_preferences(
        ...
        for content_type in content_types:
            ...
            # Combine factors
            preference = (
                base_preference + interaction_frequency * 0.3 + satisfaction_score * 0.2
            )
    ...
    async def _analyze_theme_preferences(
        ...
        for theme in themes:
            ...
            time_spent = await self._get_time_spent_in_theme(
                player_id, theme, timeframe_days
            )
            exploration_depth = await self._get_theme_exploration_depth(
                player_id, theme
            )

            preference = (
                time_spent * 0.4 + exploration_depth * 0.3 + 0.3
            )  # Base preference
    ...
    async def _analyze_difficulty_preference(
        ...
        quest_completion_rate = await self._get_quest_completion_rate(
            player_id, timeframe_days
        )
    ...
    async def _analyze_interaction_style(
        ...
        social_interactions = await self._count_social_interactions(
            player_id, timeframe_days
        )
        combat_interactions = await self._count_combat_interactions(
            player_id, timeframe_days
        )
        puzzle_interactions = await self._count_puzzle_interactions(
            player_id, timeframe_days
        )

        if (
            social_interactions > combat_interactions
            and social_interactions > puzzle_interactions
        ):
            return "social"
    ...
    async def _calculate_confidence_scores(
        ...
        )  # 100 data points = full confidence
    ...
    async def _generate_interest_reasoning(
        ...
        if interest_score > 0.7:
            ...
            reasoning.append(
                f"Moderate interest in {content_type} based on past behavior"
            )
        else:
    ...
    async def _get_quest_completion_rate(
        self, player_id: UUID, timeframe_days: int
    ) -> float:
    ...
    async def _count_social_interactions(
        self, player_id: UUID, timeframe_days: int
    ) -> int:
        """Count social interactions."""
        return 10  # Mock value

    async def _count_combat_interactions(
        self, player_id: UUID, timeframe_days: int
    ) -> int:
        """Count combat interactions."""
        return 5  # Mock value

    async def _count_puzzle_interactions(
        self, player_id: UUID, timeframe_days: int
    ) -> int:
