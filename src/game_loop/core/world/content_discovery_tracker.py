...
class ContentDiscoveryTracker:
    ...
    async def track_content_discovery(self, discovery_event: DiscoveryEvent) -> bool:
        ...
        try:
            ...
            if not await self._validate_discovery_event(discovery_event):
                logger.warning(
                    f"Invalid discovery event: {discovery_event.discovery_id}"
                )
                return False
            ...
            if discovery_event.time_to_discovery_seconds is not None:
                ...
                await self._update_discovery_difficulty(
                    discovery_event.discovery_id, difficulty
                )
            ...
            logger.info(
                f"Tracked discovery of {discovery_event.content_type} content by player {discovery_event.player_id}"
            )
            return True
    ...
    async def track_content_interaction(
        self, interaction_event: InteractionEvent
    ) -> bool:
        """
        ...
        try:
            ...
            if not await self._validate_interaction_event(interaction_event):
                logger.warning(
                    f"Invalid interaction event: {interaction_event.interaction_id}"
                )
                return False
            ...
            logger.info(
                f"Tracked interaction with {interaction_event.content_id} by player {interaction_event.player_id}"
            )
            return True
    ...
    async def analyze_discovery_patterns(
        ...
        try:
            ...
            discovery_events = await self._get_discovery_events(
            ...
            if discovery_events:
                ...
                patterns.discovery_success_rate = (
                    successful_discoveries / total_discoveries
                )
    ...
    async def get_content_effectiveness(self, content_id: UUID) -> ContentEffectiveness:
        ...
        try:
            ...
            total_players_encountered = await self._get_players_who_encountered_content(
                content_id
            )

            if total_players_encountered > 0:
                # Calculate discovery rate
                effectiveness.discovery_rate = (
                    len(discoveries) / total_players_encountered
                )
                ...
                if discoveries:
                    effectiveness.interaction_rate = len(interactions) / len(
                        discoveries
                    )
                ...
                if satisfactions:
                    effectiveness.average_satisfaction = (
                        sum(satisfactions) / len(satisfactions) / 5.0
                    )  # Normalize to 0-1
                ...
                if interactions:
                    effectiveness.completion_rate = completed_interactions / len(
                        interactions
                    )
                ...
                repeated_players = len(
                    [count for count in player_interaction_counts.values() if count > 1]
                )
                if player_interaction_counts:
                    effectiveness.replay_value = repeated_players / len(
                        player_interaction_counts
                    )
        ...
        try:
            ...
            for content in location_content:
                if content["content_id"] not in discovered_content_ids:
                    ...
                    difficulty = await self.calculate_discovery_difficulty(
                        content["content_id"]
                    )
                    ...
                    time_since_generation = await self._get_time_since_generation(
                        content["content_id"]
                    )
            ...
            logger.info(
                f"Found {len(undiscovered)} undiscovered content items for player {player_id}"
            )
            return undiscovered
    ...
    async def calculate_discovery_difficulty(self, content_id: UUID) -> float:
        ...
        try:
            ...
            for discovery in discoveries:
                adjustment = method_difficulty_adjustments.get(
                    discovery.discovery_method, 0.0
                )
    ...
    async def get_discovery_analytics(self) -> DiscoveryAnalytics:
        ...
        try:
            ...
            analytics.total_content_generated = (
                await self._count_total_generated_content()
            )
            analytics.total_content_discovered = (
                await self._count_total_discovered_content()
            )

            # Calculate discovery rates by content type
            content_types = ["location", "npc", "object", "connection", "quest"]
            for content_type in content_types:
                generated_count = await self._count_generated_content_by_type(
                    content_type
                )
                discovered_count = await self._count_discovered_content_by_type(
                    content_type
                )
    ...
    async def _calculate_discovery_difficulty_from_time(
        ...
        difficulty = min(
            1.0, time_seconds / (expected_time * 2)
        )  # 2x expected = max difficulty

        return difficulty

    async def _update_discovery_difficulty(
        self, discovery_id: UUID, difficulty: float
    ) -> None:
    ...
    async def _get_discovery_events(
    ...
    async def _get_content_interactions(
