...
async def sample_exchanges_with_embeddings(db_session, sample_conversation):
    ...
    for i, (embedding, message) in enumerate(
        zip(test_embeddings, messages, strict=False)
    ):
...
class TestSemanticMemoryRepository:
    ...
    async def test_get_memories_with_embeddings_batch(
        self, db_session, sample_exchanges_with_embeddings
    ):
        ...
        high_conf_memories = await repo.get_memories_with_embeddings_batch(
            min_confidence=0.7, batch_size=10
        )
    ...
    async def test_stream_memories_for_clustering(
        self, db_session, sample_exchanges_with_embeddings
    ):
        ...
        all_streamed_ids = {
            exchange.exchange_id for batch in batches for exchange in batch
        }
        original_ids = {
            exchange.exchange_id for exchange in sample_exchanges_with_embeddings
        }
        assert all_streamed_ids == original_ids

    async def test_get_clustering_data_preparation(
        self, db_session, sample_exchanges_with_embeddings, sample_conversation
    ):
        """Test K-means clustering data preparation."""
        repo = SemanticMemoryRepository(db_session)

        data = await repo.get_clustering_data_preparation(
            npc_id=sample_conversation.npc_id, min_cluster_size=2, max_memories=1000
        )
    ...
    async def test_batch_update_confidence_scores(
        self, db_session, sample_exchanges_with_embeddings
    ):
    ...
    async def test_batch_update_cluster_assignments(
        self, db_session, sample_exchanges_with_embeddings
    ):
        ...
        assert (
            sample_exchanges_with_embeddings[0].memory_cluster_id == cluster.cluster_id
        )
        assert (
            sample_exchanges_with_embeddings[1].memory_cluster_id == cluster.cluster_id
        )
        assert (
            float(sample_exchanges_with_embeddings[0].cluster_confidence_score) == 0.9
        )

    async def test_batch_increment_access_counts(
        self, db_session, sample_exchanges_with_embeddings
    ):
    ...
    async def test_find_similar_memories_with_filters(
        self, db_session, sample_exchanges_with_embeddings, sample_conversation
    ):
    ...
    async def test_get_repository_health_metrics(
        self, db_session, sample_exchanges_with_embeddings
    ):
    ...
    async def test_get_temporal_memory_distribution(
        self, db_session, sample_exchanges_with_embeddings, sample_conversation
    ):
        ...
        distribution = await repo.get_temporal_memory_distribution(
            npc_id=sample_conversation.npc_id, days_back=7, bucket_hours=24
        )
    ...
    async def test_empty_results_handling(self, db_session):
        ...
        memories = await repo.get_memories_with_embeddings_batch(
            npc_id=non_existent_npc
        )
        assert memories == []

        data = await repo.get_clustering_data_preparation(
            npc_id=non_existent_npc, min_cluster_size=5
        )
    ...
    async def test_performance_constraints(
        ...
        await repo.find_similar_memories(
            query_embedding=query_embedding, similarity_threshold=0.7, limit=10
        )
    ...
    async def test_input_validation_security(self, db_session):
        ...
        with pytest.raises(
            ValueError, match="Cluster ID must be a non-negative integer"
        ):
    ...
    async def test_batch_operations_with_parameterized_queries(
        self, db_session, sample_exchanges_with_embeddings
    ):
