...
class TestObjectInteractionSystem:
    ...
    def interaction_system(self):
        ...
        system = ObjectInteractionSystem(object_manager, physics_engine, skill_manager, recipe_manager)
        return system
    ...
    @pytest.mark.asyncio
    async def test_validate_interaction_requirements_insufficient_energy(self, interaction_system):
    ...
    @pytest.mark.asyncio
    async def test_validate_interaction_requirements_missing_skills(self, interaction_system):
    ...
    async def test_get_available_interactions(self, interaction_system):
        ...
        examine_interaction = next((i for i in interactions if i["type"] == ObjectInteractionType.EXAMINE), None)
        assert examine_interaction is not None
        
        # Should have use since object is usable
        use_interaction = next((i for i in interactions if i["type"] == ObjectInteractionType.USE), None)
    ...
    async def test_execute_tool_interaction_incompatible(self, interaction_system):
        ...
        interaction_system.objects.get_object_properties = AsyncMock(side_effect=lambda obj_id: {
            "hammer": {
                "name": "Hammer",
    ...
    async def test_calculate_interaction_success_probability(self, interaction_system):
        ...
        probability = await interaction_system.calculate_interaction_success_probability(
            ObjectInteractionType.USE,
            ["test_object"],
            {"tool_use": 5}
        )
    ...
    async def test_register_interaction_handler(self, interaction_system):
        ...
        assert ObjectInteractionType.TRANSFORM in interaction_system._interaction_handlers
        handlers = interaction_system._interaction_handlers[ObjectInteractionType.TRANSFORM]
    ...
    async def test_interaction_with_consumable_object(self, interaction_system):
        ...
        interaction_system.objects.get_object_properties = AsyncMock(return_value={
            "name": "Health Potion",
            "usable": True,
            "consumable": True,
        ...
        consume_interaction = next((i for i in interactions if i["type"] == ObjectInteractionType.CONSUME), None)
    ...
    async def test_object_condition_impact_on_success(self, interaction_system):
        ...
        interaction_system._get_object_condition = AsyncMock(return_value=0.2)  # Poor condition
    ...
    async def test_compatibility_matrix_caching(self, interaction_system):
        ...
        compatibility1 = await interaction_system._check_object_compatibility("obj1", "obj2")
        
        # Add to cache manually to test cache retrieval
        interaction_system._compatibility_matrix["obj1:obj2"] = {"compatibility": 0.8}
        
        # Second call should use cached value
        compatibility2 = await interaction_system._check_object_compatibility("obj1", "obj2")
