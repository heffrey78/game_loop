...
class TestObjectConditionManager:
    ...
    def condition_manager(self):
        ...
        manager = ObjectConditionManager(
            object_manager, time_manager, environment_manager
        )
        return manager
    ...
    @pytest.mark.asyncio
    async def test_track_object_condition_with_initial_condition(
    ...
    async def test_update_object_condition(self, condition_manager):
        ...
        usage_data = {"intensity": 0.5, "frequency": 0.3}
        environmental_factors = {"temperature": 25.0, "humidity": 60.0}
    ...
    async def test_calculate_condition_impact_tool_usage(self, condition_manager):
        ...
        assert (
            impact["efficiency_modifier"] <= 1.0
        )  # Should be reduced due to condition
    ...
    async def test_repair_object_already_perfect(self, condition_manager):
        ...
        success, result = await condition_manager.repair_object(
            "test_object", ["repair_kit"], 5, ["hammer"]
        )
    ...
    async def test_maintain_object(self, condition_manager):
        ...
        result = await condition_manager.maintain_object(
            "test_object", "cleaning", ["soap", "water"]
        )
    ...
    async def test_get_condition_description_basic(self, condition_manager):
        ...
        description = await condition_manager.get_condition_description(
            "test_object", "basic"
        )
    ...
    async def test_apply_condition_modifiers(self, condition_manager):
        ...
        modifiers = {"magical_enhancement": 0.1, "environmental_protection": 0.05}
    ...
    async def test_register_degradation_model(self, condition_manager):
        ...
        condition_manager.register_degradation_model(
            "custom_type", custom_degradation_model
        )

        assert "custom_type" in condition_manager._degradation_models
        assert (
            condition_manager._degradation_models["custom_type"]
            == custom_degradation_model
        )
    ...
    async def test_repair_with_insufficient_materials(self, condition_manager):
        ...
        condition_manager._validate_repair_materials = AsyncMock(
            return_value=(False, "Insufficient materials")
        )
