...
class TestNPCContextCollector:
    ...
    @pytest.mark.asyncio
    async def test_collect_generation_context_basic(self, context_collector, world_state):
    ...
    @pytest.mark.asyncio
    async def test_analyze_location_needs_with_existing_npcs(self, context_collector, world_state):
        """Test location needs analysis with existing NPCs."""
        ws, main_location_id, _ = world_state
        location = ws.locations[main_location_id]
        
        # Add some existing NPCs
        npc1 = NonPlayerCharacter(npc_id=uuid4(), name="Village Guard", description="A guard")
        npc2 = NonPlayerCharacter(npc_id=uuid4(), name="Local Merchant", description="A trader")
    ...
    @pytest.mark.asyncio
    async def test_gather_social_context_empty_location(self, context_collector, world_state):
    ...
    @pytest.mark.asyncio
    async def test_gather_social_context_with_npcs(self, context_collector, world_state):
        """Test social context gathering with NPCs."""
        ws, main_location_id, _ = world_state
        location = ws.locations[main_location_id]
        
        # Add NPCs
        guard = NonPlayerCharacter(npc_id=uuid4(), name="Village Guard", description="A guard")
        merchant = NonPlayerCharacter(npc_id=uuid4(), name="Merchant Bob", description="A trader")
        smith = NonPlayerCharacter(npc_id=uuid4(), name="Blacksmith", description="A smith")
    ...
    async def test_collect_world_knowledge(self, context_collector, world_state):
        ...
        assert "communal" in cultural["regional_characteristics"]  # Village characteristic
    ...
    async def test_get_nearby_npcs(self, context_collector, world_state):
        ...
        main_npc = NonPlayerCharacter(npc_id=uuid4(), name="Main NPC", description="In main location")
        connected_npc = NonPlayerCharacter(npc_id=uuid4(), name="Connected NPC", description="In connected location")
    ...
    @pytest.mark.asyncio
    async def test_gather_generation_constraints_village(self, context_collector, world_state):
    ...
    @pytest.mark.asyncio
    async def test_gather_generation_constraints_quest_related(self, context_collector, world_state):
    ...
    def test_get_regional_characteristics_unknown(self, context_collector):
        ...
        characteristics = context_collector._get_regional_characteristics("UnknownTheme")
    ...
    def test_get_common_knowledge_forest(self, context_collector):
        ...
        expected = ["local_wildlife", "safe_paths", "seasonal_changes", "natural_resources"]
        assert knowledge == expected
