...
{% endmacro %}
    ...
    def test_confidence_levels_affect_language(self, temp_dir, setup_memory_templates):
        ...
        assert any(word in high_conf.lower() for word in ["clearly", "certain", "remember"])
        # Low confidence should be more tentative
        assert any(word in low_conf.lower() for word in ["think", "might", "seems"])

    def test_personality_archetype_affects_language(self, temp_dir, setup_memory_templates):
    ...
    def test_trust_based_revelation(self, temp_dir, setup_memory_templates):
        ...
        assert "let you in" in confidant_result.lower() or "secret" in confidant_result.lower()
    ...
    def test_trust_level_progression(self, temp_dir, setup_memory_templates):
        ...
        high_trust_has_indicators = any(indicator in high_trust_result.lower() 
    ...
    def test_validation_quality_assessment(self, memory_template_manager):
        ...
        validation = memory_template_manager.validate_memory_reference_quality(good_reference)
        ...
        poor_validation = memory_template_manager.validate_memory_reference_quality(poor_reference)
        
        assert poor_validation["overall_quality"] < 0.7
        assert poor_validation["repetition_check"] is False

    def test_pattern_variety_and_anti_repetition(self, temp_dir, setup_memory_templates):
    ...
    def test_fallback_behavior(self, temp_dir):
        ...
        result = manager.generate_memory_reference(
            memory_content="something happened",
            confidence=0.8
        )
    ...
    def test_template_cache_functionality(self, temp_dir, setup_memory_templates):
        ...
        expected_templates = ["confidence_patterns", "personality_styles", "trust_revelation"]
    ...
    def test_confidence_categories(self, temp_dir, setup_memory_templates, confidence, expected_quality):
        ...
        if expected_quality == "high":
            assert any(word in result.lower() for word in ["clearly", "certain", "definite"])
    ...
    def test_memory_sensitivity_filtering(self, temp_dir, setup_memory_templates):
        ...
        for sensitivity, trust, should_allow in test_cases:
            ...
            if should_allow:
                assert result is not None, f"Should allow {sensitivity} at trust {trust}"
            else:
