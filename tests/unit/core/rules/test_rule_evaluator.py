...
class TestRuleEvaluator:
    ...
    def test_evaluate_rule_disabled(self, evaluator, sample_context):
        ...
        rule = Rule(
            ...
            actions=[
                RuleAction(
                    action_type=ActionType.SEND_MESSAGE, parameters={"message": "Test"}
                )
            ],
        )
...
class TestConditionEvaluators:
    ...
    def test_not_in_condition(self, evaluator):
        ...
        assert (
            evaluator._evaluate_not_in("grape", ["apple", "banana", "cherry"]) is True
        )
        assert (
            evaluator._evaluate_not_in("apple", ["apple", "banana", "cherry"]) is False
        )
    ...
    def test_regex_match_condition(self, evaluator):
        ...
        assert (
            evaluator._evaluate_regex_match("test@example.com", r".*@.*\.com") is True
        )
        assert evaluator._evaluate_regex_match("invalid_email", r".*@.*\.com") is False
        assert (
            evaluator._evaluate_regex_match("test", r"[invalid") is False
        )  # Invalid regex
...
class TestActionExecutors:
    ...
    def test_execute_send_message_action(self, evaluator, sample_context):
        ...
        action = RuleAction(
            action_type=ActionType.SEND_MESSAGE, parameters={"message": "Test message"}
        )
    ...
    def test_execute_block_action(self, evaluator, sample_context):
        ...
        action = RuleAction(
            action_type=ActionType.BLOCK_ACTION, parameters={"reason": "Action blocked"}
        )
    ...
    def test_execute_custom_action(self, evaluator, sample_context):
        ...
        action = RuleAction(
            action_type=ActionType.CUSTOM, parameters={"handler": "custom_handler"}
        )
...
class TestRuleEvaluationWithComplexContext:
    ...
    def test_rule_evaluation_error_handling(self, evaluator):
        ...
        context = RuleEvaluationContext(player_state={"health": 50})
        ...
        )  # No error because missing path just returns None
