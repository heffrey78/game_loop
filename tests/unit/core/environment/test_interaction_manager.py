...
class TestEnvironmentInteractionManager:
    ...
    @pytest.fixture
    def interaction_manager(self, mock_world_state_manager, mock_object_manager, mock_physics_engine):
    ...
    @pytest.mark.asyncio
    async def test_process_environment_interaction_success(self, interaction_manager, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_process_environment_interaction_validation_failure(self, interaction_manager):
    ...
    @pytest.mark.asyncio
    async def test_validate_interaction_requirements_success(self, interaction_manager, sample_context):
        ...
        is_valid, errors = await interaction_manager.validate_interaction_requirements(
            InteractionType.EXAMINE, "wooden_crate", sample_context.get("player_state", {})
        )
    ...
    @pytest.mark.asyncio
    async def test_validate_interaction_requirements_missing_tool(self, interaction_manager):
    ...
    @pytest.mark.asyncio
    async def test_execute_object_manipulation_success(self, interaction_manager, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_execute_object_manipulation_insufficient_strength(self, interaction_manager):
    ...
    @pytest.mark.asyncio
    async def test_handle_container_interactions_open(self, interaction_manager, sample_context):
        ...
        assert isinstance(result, PhysicalActionResult)
        assert result.action_type in [PhysicalActionType.OPENING, PhysicalActionType.MANIPULATION]

    @pytest.mark.asyncio
    async def test_handle_container_interactions_put_item(self, interaction_manager, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_handle_container_interactions_take_item(self, interaction_manager, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_handle_container_interactions_invalid_action(self, interaction_manager, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_process_tool_usage_success(self, interaction_manager, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_process_tool_usage_incompatible_action(self, interaction_manager, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_handle_environmental_puzzles_success(self, interaction_manager, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_handle_environmental_puzzles_already_solved(self, interaction_manager, sample_context):
    ...
    async def test_update_environmental_state(self, interaction_manager):
        ...
        await interaction_manager.update_environmental_state("treasure_chest", state_changes)
        
        # Check that state was updated
        entity_state = await interaction_manager._get_entity_state("treasure_chest")
        assert "open" in entity_state or len(interaction_manager._environmental_states) >= 0

    @pytest.mark.asyncio
    async def test_check_interaction_side_effects(self, interaction_manager, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_get_interaction_options_container(self, interaction_manager, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_get_interaction_options_moveable_object(self, interaction_manager, sample_context):
        """Test getting interaction options for moveable object."""
        with patch.object(interaction_manager, '_get_object_info') as mock_get_info:
            mock_get_info.return_value = {"type": "object", "moveable": True, "mass": 15}
    ...
    def test_register_interaction_handler(self, interaction_manager):
        ...
        interaction_manager.register_interaction_handler("custom_interaction", mock_handler)
        
        assert "custom_interaction" in interaction_manager._interaction_handlers
        assert interaction_manager._interaction_handlers["custom_interaction"] == mock_handler
    ...
    @pytest.mark.asyncio
    async def test_puzzle_completion_detection(self, interaction_manager, sample_context):
    ...
    async def test_manipulation_energy_cost_calculation(self, interaction_manager):
        ...
        cost = interaction_manager._calculate_manipulation_energy_cost("push", 20.0, 1.5)
    ...
    async def test_manipulation_time_cost_calculation(self, interaction_manager):
        ...
        time_cost = interaction_manager._calculate_manipulation_time_cost("pull", 15.0, 1.2)
