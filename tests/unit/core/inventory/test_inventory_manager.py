...
class TestInventoryManager:
    ...
    async def test_create_inventory(self, inventory_manager):
        ...
        inventory_id = await inventory_manager.create_inventory("test_player", "default")
    ...
    async def test_add_item_success(self, inventory_manager, test_inventory):
        ...
        success, result = await inventory_manager.add_item(test_inventory, "test_item", 1)
    ...
    async def test_add_item_stacking(self, inventory_manager, test_inventory):
        ...
        success, result = await inventory_manager.add_item(test_inventory, "test_item", 2)
    ...
    async def test_remove_item_success(self, inventory_manager, test_inventory):
        ...
        # Remove some items
        success, result = await inventory_manager.remove_item(test_inventory, "test_item", 3)
    ...
    async def test_remove_item_not_found(self, inventory_manager, test_inventory):
        ...
        success, result = await inventory_manager.remove_item(test_inventory, "nonexistent_item", 1)
        
        assert success is False
        assert "not found" in result["error"]

    @pytest.mark.asyncio
    async def test_remove_item_insufficient_quantity(self, inventory_manager, test_inventory):
        """Test removing more items than available."""
        await inventory_manager.add_item(test_inventory, "test_item", 2)
        
        success, result = await inventory_manager.remove_item(test_inventory, "test_item", 5)
    ...
    async def test_weight_constraint_validation(self, inventory_manager):
        ...
        # Mock heavy item
        inventory_manager.objects.get_object_properties = AsyncMock(return_value={
            "name": "Heavy Item",
            "weight": 5.0,
            "volume": 1.0,
            "category": "misc"
        ...
        success, result = await inventory_manager.add_item(inventory_id, "heavy_item", 1)
    ...
    async def test_custom_constraint_validator(self, inventory_manager):
        ...
        assert InventoryConstraintType.SPECIAL in inventory_manager._constraint_validators
    ...
    async def test_error_handling_invalid_inventory(self, inventory_manager):
        ...
        success, result = await inventory_manager.add_item("invalid_inventory", "item", 1)
