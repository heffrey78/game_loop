...
class TestPhysicalActionIntegration:
    ...
    def mock_physical_processor(self):
        ...
        mock_processor = Mock()
        mock_processor.process_physical_action = AsyncMock(return_value=PhysicalActionResult(
            success=True,
        ...
        mock_processor.validate_action_feasibility = AsyncMock(return_value=(True, None))
    ...
    def mock_movement_manager(self):
        ...
        mock_movement = Mock()
        mock_movement.process_movement_command = AsyncMock(return_value=PhysicalActionResult(
            success=True,
    ...
    def mock_environment_manager(self):
        ...
        mock_environment.process_environment_interaction = AsyncMock(return_value=PhysicalActionResult(
            success=True,
            action_type=PhysicalActionType.MANIPULATION,
    ...
    @pytest.fixture
    def integration_layer(self, mock_physical_processor, mock_movement_manager, 
                         mock_environment_manager, mock_physics_engine, 
                         mock_spatial_navigator, mock_game_state_manager):
    ...
    @pytest.mark.asyncio
    async def test_process_classified_physical_action_success(self, integration_layer, 
    ...
    @pytest.mark.asyncio
    async def test_process_classified_physical_action_feasibility_failure(self, integration_layer):
        ...
        result = await integration_layer.process_classified_physical_action(action, context)
    ...
    @pytest.mark.asyncio
    async def test_coordinate_multi_step_actions_success(self, integration_layer, sample_context):
        ...
        results = await integration_layer.coordinate_multi_step_actions(action_sequence, sample_context)
    ...
    @pytest.mark.asyncio
    async def test_coordinate_multi_step_actions_with_failure(self, integration_layer, sample_context):
        ...
        failing_processor.process_physical_action = AsyncMock(side_effect=[
            PhysicalActionResult(
                success=True, action_type=PhysicalActionType.OPENING,
                affected_entities=["door"], state_changes={"door_open": True},
                energy_cost=5.0, time_elapsed=2.0, side_effects=[], description="Door opened."
            ),
            PhysicalActionResult(
                success=False, action_type=PhysicalActionType.MOVEMENT,
                affected_entities=["player"], state_changes={},
                energy_cost=0.0, time_elapsed=0.0, side_effects=[],
                description="Movement failed.", error_message="Path blocked"
            ),
        ])
        
        failing_processor.validate_action_feasibility = AsyncMock(return_value=(True, None))
        integration_layer.execution_context.physical_processor = failing_processor
        
        action_sequence = [
            {"classification": ActionClassification(action_type=ActionType.PHYSICAL, confidence=0.9, target="door")},
            {"classification": ActionClassification(action_type=ActionType.PHYSICAL, confidence=0.9, target="north")},
        ]
    ...
    @pytest.mark.asyncio
    async def test_handle_action_interruptions_continue(self, integration_layer, sample_context):
        """Test action interruption handling when action can continue."""
        active_action = {
            "classification": ActionClassification(action_type=ActionType.PHYSICAL, confidence=0.9, target="climb_rope"),
            "start_time": 1000.0,
            "progress": 0.6,
        }
    ...
    @pytest.mark.asyncio
    async def test_handle_action_interruptions_stop(self, integration_layer, sample_context):
        """Test action interruption handling when action must stop."""
        active_action = {
            "classification": ActionClassification(action_type=ActionType.PHYSICAL, confidence=0.9, target="heavy_lift"),
            "start_time": 1000.0,
            "progress": 0.3,
        }
    ...
    @pytest.mark.asyncio
    async def test_validate_action_chain_feasibility_success(self, integration_layer, sample_context):
        ...
        is_feasible, failing_step = await integration_layer.validate_action_chain_feasibility(
    ...
    @pytest.mark.asyncio
    async def test_handle_concurrent_actions_success(self, integration_layer, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_handle_concurrent_actions_with_conflicts(self, integration_layer, sample_context):
        """Test concurrent action handling with conflicts."""
        # Mock conflict detection
        integration_layer._detect_action_conflicts = AsyncMock(return_value=[
            {"type": "resource_conflict", "actions": [0, 1]}
        ])
        integration_layer._resolve_action_conflicts = AsyncMock(return_value=[
            {"classification": ActionClassification(action_type=ActionType.PHYSICAL, confidence=0.9, target="resolved")}
        ])
        
        conflicting_actions = [
            {"classification": ActionClassification(action_type=ActionType.PHYSICAL, confidence=0.9, target="same_target")},
            {"classification": ActionClassification(action_type=ActionType.PHYSICAL, confidence=0.9, target="same_target")},
        ]
    ...
    async def test_generate_action_feedback(self, integration_layer, sample_context):
        ...
        feedback = await integration_layer.generate_action_feedback(action_result, sample_context)
    ...
    @pytest.mark.asyncio
    async def test_execution_strategy_determination(self, integration_layer, sample_context):
        ...
        strategy = await integration_layer._determine_execution_strategy(movement_action, sample_context)
        ...
        strategy = await integration_layer._determine_execution_strategy(environment_action, sample_context)
    ...
    @pytest.mark.asyncio
    async def test_environment_action_execution(self, integration_layer, sample_context):
    ...
    async def test_action_result_conversion(self, integration_layer):
        ...
        # Convert to dict
        result_dict = await integration_layer._convert_action_result_to_dict(action_result)
        assert isinstance(result_dict, dict)
        assert result_dict["success"] is True
        assert result_dict["action_type"] in ["manipulation", PhysicalActionType.MANIPULATION]
        
        # Convert back to result
        converted_result = await integration_layer._convert_dict_to_action_result(result_dict)
    ...
    async def test_action_type_extraction(self, integration_layer):
        ...
        action_type = await integration_layer._extract_physical_action_type(movement_classification)
        ...
        action_type = await integration_layer._extract_physical_action_type(push_classification)
    ...
    @pytest.mark.asyncio
    async def test_action_metrics_update(self, integration_layer, sample_action_classification):
        """Test action metrics updating."""
        result = {"success": True, "time_elapsed": 8.0}
        
        await integration_layer._update_integration_metrics(sample_action_classification, result)
    ...
    @pytest.mark.asyncio
    async def test_hook_execution(self, integration_layer, sample_action_classification, sample_context):
...
async def test_integration_full_workflow():
    ...
    # Create all mocks
    mock_physical = Mock()
    mock_physical.process_physical_action = AsyncMock(return_value=PhysicalActionResult(
        success=True, action_type=PhysicalActionType.MANIPULATION,
        affected_entities=["test"], state_changes={}, energy_cost=10.0,
        time_elapsed=5.0, side_effects=[], description="Success"
    ))
    ...
    integration = PhysicalActionIntegration(
        mock_physical, mock_movement, mock_environment,
        mock_physics, mock_navigator, mock_state
    )
